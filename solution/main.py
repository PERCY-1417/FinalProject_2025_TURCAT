...
with open(os.path.join(dataset_train_dir, "args.txt"), "w") as f:
    f.write(
        "\n".join(
            [
                str(k) + "," + str(v)
...
if __name__ == "__main__":
    ...
    if args.state_dict_path is not None:
        try:
            ...
            if (
                not args.generate_recommendations
            ):  # Allow proceeding without weights if not generating recs (i.e. for training)
                import pdb

                pdb.set_trace()
            else:
                print(
                    "Cannot generate recommendations without a loaded model. Exiting."
                )
                exit()
    elif args.generate_recommendations:
        print(
            "Error: --generate_recommendations mode requires --state_dict_path to be provided. Exiting."
        )
        exit()
    ...
    if args.generate_recommendations:
        ...
        for user_id in range(1, usernum + 1):
            ...
            if (
                not user_full_history
            ):  # Or based on user_train, if we only want to use training sequence
            ...
            scores = model.predict(
                np.array([user_id]), log_seqs_np, item_indices_np
            )  # scores for items in item_indices_np

            # scores is a tensor on device, move to cpu and numpy
            scores_np = (
                scores.cpu().detach().numpy().flatten()
            )  # Shape (len(items_to_score),)
        ...
        # import json
        # with open(os.path.join(dataset_train_dir, 'recommendations.json'), 'w') as f_recs:
        #     json.dump(recommendations, f_recs)
        # print(f"Recommendations saved to {os.path.join(dataset_train_dir, 'recommendations.json')}")
        ...
        f = open(os.path.join(dataset_train_dir, "log.txt"), "w")
        f.write(
        ...
        epoch_start_idx = 1
        if (
            args.state_dict_path is not None
        ):  # This check is already done above and model loaded
            ...
            if not args.generate_recommendations:
                try:
                    ...
                    print(
                        "Failed to parse epoch from state_dict_path for training resumption."
                    )
        ...
        for epoch in range(epoch_start_idx, args.num_epochs + 1):
            ...
            if epoch % 20 == 0:
                ...
                if (
                    t_valid[2] > best_val_recall
                    or t_valid[0] > best_val_ndcg
                    or t_test[2] > best_test_recall
                    or t_test[0] > best_test_ndcg
                ):

                    best_val_ndcg = max(t_valid[0], best_val_ndcg)
                    best_val_precision = max(
                        t_valid[1], best_val_precision
                    )  # Keep track of best P too
